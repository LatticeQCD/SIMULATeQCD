...
void AdvancedMultiShiftCG<floatT, NStacks>::invert_mixed(LinearOperator<SpinorIn_t>& dslash, LinearOperator<Spinor_t_half>&  dslash_half,
        SpinorOut_t& spinorOut, const SpinorIn_t& spinorIn,SimpleArray<floatT, NStacks> sigma, const int max_iter, const double precision)
{
    ...
    Spinor_t_half pi0_half(spinorIn.getComm());
    Spinor_t_half s_half(spinorIn.getComm());
    ...
    do {
        ...
        pi0_half.template convert_precision<floatT>(pi0);
        pi0_half.updateAll(COMM_BOTH | Hyperplane);

        dslash_half.applyMdaggM(s_half, pi0_half, false);
        s.template convert_precision<__half>(s_half);
        ...
        accum.template axpyThisLoop<32>(((floatT)(-1.0))*B, pi, max_term);
...
template<class floatT, size_t NStacks>
template <typename Spinor_t>
void ConjugateGradient<floatT, NStacks>::invert_new(LinearOperator<Spinor_t>& dslash, Spinor_t& spinorOut, const Spinor_t& spinorIn,
...
template<class floatT, size_t NStacks>
template <typename Spinor_t>
void ConjugateGradient<floatT, NStacks>::invert_res_replace(LinearOperator<Spinor_t>& dslash, Spinor_t& spinorOut,
        const Spinor_t& spinorIn, const int max_iter, const double precision)
{
    Spinor_t pi(spinorIn.getComm());
    Spinor_t s(spinorIn.getComm());
    Spinor_t r(spinorIn.getComm());
...
void ConjugateGradient<floatT, NStacks>::invert_mixed(LinearOperator<Spinor_t>& dslash, LinearOperator<Spinor_t_half>& dslash_half,
        Spinor_t& spinorOut, const Spinor_t& spinorIn, const int max_iter, const double precision)
{
    Spinor_t pi(spinorIn.getComm());
    Spinor_t s(spinorIn.getComm());
    Spinor_t r(spinorIn.getComm());

    Spinor_t_half pi_half(spinorIn.getComm());
    Spinor_t_half s_half(spinorIn.getComm());
    
    bool do_half_prec = true;
    int cg = 0;
    ...
    do {
        ...
        if (do_half_prec) {
            pi_half.template convert_precision<floatT>(pi);
            
            dslash_half.applyMdaggM(s_half,pi_half,false);
            s.template convert_precision<__half>(s_half);
        } else {
            dslash.applyMdaggM(s, pi, false);
        }
        ...
        rootLogger.debug() << "CG-Mixed residue: " << max(lambda2/in_norm);
        if (max(norm_r2) < 1e-1*max(norm_restart)) {
                //recompute precise residual to avoid drift
                ...
                dot3 = r.dotProductStacked(r);
                pi = r;
                norm_restart = real<double>(dot3);

                norm_r2 = norm_restart;
                lambda2 = norm_restart;
                rootLogger.debug() << "recompute precise residual " << max(norm_restart/in_norm);
            }

        if (do_half_prec && max(lambda2/in_norm) < precision) { //check if we have actually converged or if we drifted off too far since last restart... Really necessary?
            do_half_prec = false;
            rootLogger.debug() << "finished half prec iterations!";
            r = spinorIn;
...
template<class floatT, size_t NStacks>
template<typename Spinor_t, typename Spinor_t_half>
void ConjugateGradient<floatT, NStacks>::invert_mrel(LinearOperator<Spinor_t>& dslash, LinearOperator<Spinor_t_half>& dslash_half,
        Spinor_t& spinorOut, const Spinor_t& spinorIn, const int max_iter, const double precision)
{
    Spinor_t pi(spinorIn.getComm());
    Spinor_t s(spinorIn.getComm());
    ...
    do {
        ...
        pi_half.template convert_precision<floatT>(pi);
        pi_half.updateAll(COMM_BOTH | Hyperplane);
        //pAp 
        dslash_half.applyMdaggM(s_half,pi_half,false);
        s.template convert_precision<__half>(s_half);
        pi.template convert_precision<__half>(pi_half);
        dot = pi.dotProductStacked(s);
        pAp = real<double>(dot);
        alpha = -1.0 * norm_r2 / pAp;
...
template void AdvancedMultiShiftCG<floatT,STACKS>::invert_mixed(LinearOperator<Spinorfield<floatT, true, LO, HALOSPIN, 1> >& dslash, LinearOperator<Spinorfield<__half, true, LO, HALOSPIN,1> >& dslash_half, Spinorfield<floatT, true, LO, HALOSPIN, STACKS>& spinorOut, const Spinorfield<floatT, true, LO, HALOSPIN,1>& spinorIn,  SimpleArray<floatT, STACKS> sigma,const int, const double);
