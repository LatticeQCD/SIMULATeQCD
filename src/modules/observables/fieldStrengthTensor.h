...
/*Qmunu(SU3Accessor<floatT>, SU3<floatT> &,  gSite,  int ,  int )
        Determine:
...
/*FmunuKernel(SU3Accessor<floatT> , SU3<floatT> & ,  gSite ,  int ,  int )
...
struct FieldStrengthTensor {
    ...
    __device__ __host__ inline SU3<floatT> operator()(gSite site, int mu, int nu) const {
        ...
        //define a temporary SU3 for the Fmunu computation
        SU3<floatT> Fmunu;

        //define a temporary SU3 for the Qmunu computations
        SU3<floatT> Qmunu;

        Qmunu = plClov(site, mu, nu);

        // compute F_{mu,nu} = -i*1/8 * (Q_{mu,nu} - Q_{nu,mu}) - 1/3 tr(F_mu_nu)*I
        Fmunu = (COMPLEX(floatT)(0, -1)) / ((floatT) 8) * (Qmunu - dagger(Qmunu)); // this is faster...

        // return tracless F_{mu,nu}
        return Fmunu - 1. / 3. * tr_c(Fmunu) * unitySU3;
...
struct FieldStrengthTensor_imp {
    ...
    __device__ __host__ inline SU3<floatT> operator()(gSite site, int mu, int nu) const {
        ...
        //define a temporary SU3 for the Fmunu computation
        SU3<floatT> Fmunu;
        SU3<floatT> Fmunu_plaq;
        SU3<floatT> Fmunu_rect;
        ...
        Qmunu = plClov(site, mu, nu);
        // compute F_{mu,nu} = -i*1/8 * (Q_{mu,nu} - Q_{nu,mu}) - 1/3 tr(F_mu_nu)*I
        Fmunu_plaq = (COMPLEX(floatT)(0, -1)) / ((floatT) 8) * (Qmunu - dagger(Qmunu)); // this is faster...

        Qmunu = rcClov(site, mu, nu);
        Fmunu_rect = (COMPLEX(floatT)(0, -1)) / ((floatT) 16) * (Qmunu - dagger(Qmunu));


        Fmunu = floatT(5./3.) * Fmunu_plaq - floatT(1./3.) * Fmunu_rect;

        // return tracless F_{mu,nu}
        return Fmunu - floatT(1./3.) * tr_c(Fmunu) * unitySU3;
    }
};
...
struct FieldStrengthTensor_imp_imp {
    ...
    __device__ __host__ inline SU3<floatT> operator()(gSite site, int mu, int nu) const {
        ...
        if (k1 != 0.0){ //1x1
            ...
            Fmunu += (COMPLEX(floatT)(0, -1)) / ((floatT) 8) * (Qmunu - dagger(Qmunu)); 
        }
        if (k2 != 0.0){ //2x2
