...
struct WBDens {
    ...
    WBDens(gaugeAccessor<floatT> gAcc) : gAcc(gAcc), FT(gAcc) {}
        ...
        for(int nu1=1;nu1<4;++nu1){
            for(int rho=0; rho < 4; rho++){
                if((rho != mu1) && (rho != nu1)){
                    ...
                    tmp += tr_d((GCOMPLEX(floatT)(0, 1)) * ((floatT) 8) / ((floatT) 3) * (Fmu1rho * Fnu1rho - Fnu1rho*Fmu1rho) * Fmunu);
                }
            }
        }
        return tmp;
...
struct WBDens_imp {
    ...
    WBDens_imp(gaugeAccessor<floatT> gAcc) : gAcc(gAcc), FT(gAcc) {}
    __device__ __host__ inline floatT operator()(gSite site) {
...
struct WBDens_imp_imp {
    ...
    WBDens_imp_imp(gaugeAccessor<floatT> gAcc) : gAcc(gAcc), FT(gAcc) {}
    __device__ __host__ inline floatT operator()(gSite site) {
