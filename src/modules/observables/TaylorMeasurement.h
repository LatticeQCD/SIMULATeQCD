...
struct DerivativeOperatorMeasurement {
    ...
    const GCOMPLEX(double) measurement;
    const GCOMPLEX(double) std;

    DerivativeOperatorMeasurement(long operatorId, GCOMPLEX(double) measurement, GCOMPLEX(double) std) :
...
public:
    ...
    GCOMPLEX(double) measurement_accum = 0.0;
    GCOMPLEX(double) measurement_sqr_accum = 0.0;
    ...
    void collectMeasurements(std::vector<DerivativeOperatorMeasurement> &measurements, const int samples, const double normalization, const long id = 0) const {
        GCOMPLEX(double) meas = (measurement_accum / samples) / normalization;
        GCOMPLEX(double) meas_sqr = (measurement_sqr_accum / samples) / (normalization * normalization);
        // standard deviation of the mean is sqrt(sigma^2 / N)
        GCOMPLEX(double) std = GCOMPLEX(double)(sqrt(std::max(0.0, real(meas_sqr) - real(meas)*real(meas)) / (samples - 1)), sqrt(std::max(0.0, imag(meas_sqr) - imag(meas)*imag(meas)) / (samples - 1)));
...
private:
    ...
    void recursive_apply_operator_tree(std::vector<LineFormatter> *tags, DerivativeOperatorTreeNode &operatorNode, int depth) {
        ...
        for (int i = 0; i <= MAX_DERIVATIVE; i++) {
            ...
            SimpleArray<GCOMPLEX(double), NStacks> tr(0);
            ...
            for (size_t i = 0; i < NStacks; i++) {
                tr[i] = GCOMPLEX(double)(real(tr[i]) * real(tr[i]), imag(tr[i]) * imag(tr[i]));
            }
