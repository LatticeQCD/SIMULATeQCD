...
__device__ __host__ inline SU3<floatT> SU3Staple(SU3Accessor<floatT> SU3Accessor,
                                                  const gSite &site, const uint8_t &mu) {
    typedef GIndexer<LatLayout, HaloDepth> GInd;
    SU3<floatT> Ustaple, U1, U2, U3;
    Ustaple = su3_zero<floatT>();
    for (uint8_t nu = 0; nu < 4; nu++) {
        if (nu != mu) {
            gSite x_p_mu = GInd::site_up(site, mu);
            gSite x_p_nu = GInd::site_up(site, nu);
            /// Right part of the staple.
            U1 = SU3Accessor.getLink(GInd::getSiteMu(x_p_mu, nu));
            U2 = SU3Accessor.getLinkDagger(GInd::getSiteMu(x_p_nu, mu));
            U3 = SU3Accessor.getLinkDagger(GInd::getSiteMu(site, nu));
            Ustaple += U1 * U2 * U3;
            gSite x_m_nu = GInd::site_dn(site, nu);
            gSite x_p_mu_m_nu = GInd::site_dn(x_p_mu, nu);
            /// Left part of the staple.
            U1 = SU3Accessor.getLinkDagger(GInd::getSiteMu(x_p_mu_m_nu, nu));
            U2 = SU3Accessor.getLinkDagger(GInd::getSiteMu(x_m_nu, mu));
            U3 = SU3Accessor.getLink(GInd::getSiteMu(x_m_nu, nu));
            Ustaple += U1 * U2 * U3;
        }
    }
    return Ustaple;
...
__device__ __host__ inline SU3<floatT> OR_GPUSU3(const SU3<floatT> &U, const SU3<floatT> &Ustaple) {
    SU3<floatT> w;
    SU2<floatT> a, z;
    floatT znorm;
    w = U;
...
__device__ __host__ inline SU2<floatT> HB_GPUSU2(const SU2<floatT> &U, const floatT inv_4beta, uint4 *stateElem,
                                                  bool ltest)
{
    SU2<floatT> z, g;
    floatT znorm, alpha;
    ...
    if (x4 < 1.0) {
        ...
        g = SU2<floatT>(COMPLEX(floatT)(x0, x1), COMPLEX(floatT)(x2, x3));
        g = g * dagger(z);
    } else {
        g = SU2<floatT>(1.0, 0.0);
    }
...
template<typename floatT>
__device__ __host__ inline SU3<float> HB_GPUSU3(const SU3<floatT> &U, SU3<floatT> &Ustaple,
