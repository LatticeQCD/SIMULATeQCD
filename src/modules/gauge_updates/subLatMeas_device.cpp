...
template<class floatT, bool onDevice, size_t HaloDepth>
void SubLatMeas<floatT, onDevice, HaloDepth>::updateSubNorm(int pos_t, std::vector<floatT> &SubTbarbp00, std::vector<floatT> &SubSbp, std::vector<COMPLEX(floatT)> &SubTbarbc00_SubSbc) {
    ...
    for (int dist=0;dist<_sub_lt-3;dist++) {
        ...
        COMPLEX(floatT) result_tmp1(0,0);
...
struct updateSubPoly_Kernel{
    ...
    __device__ __host__ void operator()(gSite site) {
        ...
        SU3<floatT> temp1 = gAcc.getLink(GInd::getSiteMu(shifted_site, 3));
        for(int tau = 1; tau < sub_lt; tau++){
            shifted_site = GInd::site_up(shifted_site,3);
            temp1*= gAcc.getLink(GInd::getSiteMu(shifted_site, 3));
        }
        SU3<floatT> temp2;
        sub_poly_Nt.getValue<SU3<floatT>>(pos_t*elems1+Id, temp2);

        floatT factor = (1./count);
        sub_poly_Nt.setValue<SU3<floatT>>(pos_t*elems1+Id, temp1*factor+temp2);
    }
};
 ...
 |                    | and its flipped one*/
        ...
        for(int sqPos = 0; sqPos < sub_lt - 2; ++sqPos){
            for( size_t mu = 0; mu <= 2; ++mu){
                ...
                SU3<floatT> p_up = su3_one<floatT>();
                SU3<floatT> p_dn = su3_one<floatT>();
                ...
                sub1_cec_Nt.getValue<SU3<floatT>>(pos_t*(elems1*6*(sub_lt-2)) + 6*elems1*sqPos + mu*elems1 + Id1, temp1);
                floatT factor = (1./count);
                sub1_cec_Nt.setValue<SU3<floatT>>(pos_t*(elems1*6*(sub_lt-2)) + 6*elems1*sqPos + mu*elems1 + Id1, temp1 + p_up*factor);
                sub1_cec_Nt.getValue<SU3<floatT>>(pos_t*(elems1*6*(sub_lt-2)) + 6*elems1*sqPos + (mu+3)*elems1 + Id1, temp2);
                sub1_cec_Nt.setValue<SU3<floatT>>(pos_t*(elems1*6*(sub_lt-2)) + 6*elems1*sqPos + (mu+3)*elems1 + Id1, temp2 + p_dn*factor);
            }
        }
        ...
        if ( is_onBoundary ) {
            ...
            for(int sqPos = 0; sqPos < sub_lt - 2; ++sqPos){ //loop over possible square positions of sub correlator. We do not update squares at the border of the sublattice. so sqPos=0 corresponds to postion of square - left border = 1
                for( size_t mu = 0; mu <= 2; ++mu){
                    ...
                    //Distinct indexing for the combination
                    sub2_cec_Nt.getValue<SU3<floatT>>(pos_t*(elems2*6*(sub_lt-2)) + 6*elems2*sqPos + mu*elems2 + Id2, temp1);
                    floatT factor = (1./count);
                    sub2_cec_Nt.setValue<SU3<floatT>>(pos_t*(elems2*6*(sub_lt-2)) + 6*elems2*sqPos + mu*elems2 + Id2, temp1 + p_up*factor);
                    sub2_cec_Nt.getValue<SU3<floatT>>(pos_t*(elems2*6*(sub_lt-2)) + 6*elems2*sqPos + (mu+3)*elems2 + Id2, temp2);
                    sub2_cec_Nt.setValue<SU3<floatT>>(pos_t*(elems2*6*(sub_lt-2)) + 6*elems2*sqPos + (mu+3)*elems2 + Id2, temp2 + p_dn*factor);
                }
            }
        }
    }
...
struct contractPoly_Kernel{
    ...
    __device__ __host__ COMPLEX(floatT) operator()(gSite site) {
        ...
        SU3<floatT> tmp1; //sub poly in the first sublattice at some spatial site
        SU3<floatT> tmp2; //sub poly in the second sublattice at the same spatial site as tmp1
        ...
        for (int i = 0; i < Ntau; ++i) {
            ...
            for ( int j = i + min_dist; j <= i + (Ntau-2*sub_lt-min_dist); ++j) {
                ...
                SU3<floatT> p = su3_one<floatT>();
                sub_poly_Nt.getValue<SU3<floatT>>(i*elems1+Id, tmp1);
                sub_poly_Nt.getValue<SU3<floatT>>(((j+sub_lt)%Ntau)*elems1+Id, tmp2);
                ...
                for (int sqPos1 = 0; sqPos1 < sub_lt-2; ++sqPos1) {
                    ...
                    if ( 0 <= sqPos2 && sqPos2 < sub_lt-2) {
                        ...
                        for( size_t mu = 0; mu <= 2; ++mu) {
                            ...
                            sub1_cec_Nt.getValue<SU3<floatT>>(i*(6*elems1*(sub_lt-2)) + 6*elems1*sqPos1+mu*elems1+Id1, tmp1_up);
                            sub1_cec_Nt.getValue<SU3<floatT>>(i*(6*elems1*(sub_lt-2)) + 6*elems1*sqPos1+(mu+3)*elems1+Id1, tmp1_dn);
                            ...
                            if ( is_onBoundary ) {
                                ...
                                sub2_cec_Nt.getValue<SU3<floatT>>(((j+sub_lt)%Ntau)*(6*elems2*(sub_lt-2)) + 6*elems2*sqPos2+mu*elems2+Id2, tmp2_up);
                                sub2_cec_Nt.getValue<SU3<floatT>>(((j+sub_lt)%Ntau)*(6*elems2*(sub_lt-2)) + 6*elems2*sqPos2+(mu+3)*elems2+Id2, tmp2_dn);
                            } else {
                                ...
                                sub1_cec_Nt.getValue<SU3<floatT>>(((j+sub_lt)%Ntau)*(6*elems1*(sub_lt-2)) + 6*elems1*sqPos2+(mu+3)*elems1+Id3, tmp2_up);
                                sub1_cec_Nt.getValue<SU3<floatT>>(((j+sub_lt)%Ntau)*(6*elems1*(sub_lt-2)) + 6*elems1*sqPos2+mu*elems1+Id4, tmp2_dn);
                            }
...
template<class floatT, bool onDevice, size_t HaloDepth>
COMPLEX(floatT) SubLatMeas<floatT, onDevice, HaloDepth>::contraction_poly(MemoryAccessor &sub_poly_Nt, int min_dist) {

     COMPLEX(floatT) PolyakovLoop_result;
...
template<class floatT, bool onDevice, size_t HaloDepth>
std::vector<COMPLEX(floatT)> SubLatMeas<floatT, onDevice, HaloDepth>::contraction_cec(MemoryAccessor &sub1_cec_Nt, MemoryAccessor &sub2_cec_Nt, int min_dist) {
    ...
    std::vector<COMPLEX(floatT)> ColorElectricCorr_result(_Nt/2+1, 0);
