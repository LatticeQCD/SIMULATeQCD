...
protected:
    ...
    LatticeContainer<onDevice, COMPLEX(floatT)> _redBaseCE;
...
public:
    ...
    void updateSubNorm(int pos_t, std::vector<floatT> &SubTbarbp00, std::vector<floatT> &SubSbp, std::vector<COMPLEX(floatT)> &SubTbarbc00_SubSbc);

    COMPLEX(floatT) contraction_poly(MemoryAccessor &sub_poly_Nt, int min_dist);
    std::vector<COMPLEX(floatT)> contraction_cec(MemoryAccessor &sub1_cec_Nt, MemoryAccessor &sub2_cec_Nt, int min_dist);
};
