...
struct ORKernel{
    ...
    uint8_t _mu;
    ORKernel(Gaugefield<floatT,true,HaloDepth> &gauge,uint8_t mu) : SU3Accessor(gauge.getAccessor()), _mu(mu){}
...
struct HBKernel{
    ...
    HBKernel(Gaugefield<floatT,true,HaloDepth> &gauge,uint4* state,floatT beta, uint8_t mu, bool ltest) :
            SU3Accessor(gauge.getAccessor()), _state(state), _beta(beta), _mu(mu), _ltest(ltest) {}
