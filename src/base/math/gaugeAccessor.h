...
class gaugeAccessor : public GaugeConstructor<floatT_memory, comp> {

public:

    explicit gaugeAccessor(GCOMPLEX(floatT_memory) *const elements[EntryCount<comp>::count])
    ...
    __host__ __device__ inline GSU3<floatT_compute> getLinkPath(gSiteMu &siteMu, int dir, Args... args) const {
        ...
        gSite site = siteMu;
        GSU3<floatT_compute> ret = getLinkPath<LatLayout, HaloDepth, floatT_compute>(site, siteMu.mu, dir, args...);
