...
struct Matrix4x4Sym {
   ...
   /* __device__ __host__ inline Matrix4x4Sym<floatT>& operator=(const floatT &y)
    ...
    __device__ __host__ inline Matrix4x4Sym<floatT>& operator/=(floatT y)
    {
        for(int i = 0; i<10;i++){
            elems[i]/=y;
        }
...
template<class floatT>
__device__ __host__ inline Matrix4x4Sym<floatT> operator+(const Matrix4x4Sym<floatT> &x, const Matrix4x4Sym<floatT> &y) {
