...
private:
    ...
    //Revised by ranluo
    void _extractHalosSeg_async(Accessor acc, GCOMPLEX(floatT) *HaloBuffer, unsigned int param);
    void _injectHalosSeg_async(Accessor acc, GCOMPLEX(floatT) *HaloBuffer, unsigned int param);
...
public:
    ...
    void updateAll_async(unsigned int param = AllTypes | COMM_BOTH) {
        ...
        if (commtype & COMM_START) {
            ...
            if (onDevice) {
                if (!(_commBase.gpuAwareMPIAvail() || _commBase.useGpuP2P())) {
                    ...
                    _extractHalosSeg_async(getAccessor(), _haloBuffer_Device->template getPointer<GCOMPLEX(floatT) >(),
                                     param);
                }
            } else {
                _extractHalos(getAccessor(), _haloBuffer_Host->template getPointer<GCOMPLEX(floatT) >());
        ...
        if (commtype & COMM_FINISH) {
            ...
            if (onDevice) {
                if (_commBase.gpuAwareMPIAvail() || _commBase.useGpuP2P()) {
                    _injectHalosSeg_async(getAccessor(), _haloBuffer_Device_recv->template getPointer<GCOMPLEX(floatT) >(),
                                    param);
                } else {
		            HaloInfo.syncAllStreamRequests();
                    gpuErr = gpuMemcpy(_haloBuffer_Device->template getPointer<GCOMPLEX(floatT) >(),
...
void siteComm<floatT, onDevice, Accessor, AccType, EntryCount, ElemCount, LatLayout, HaloDepth>::_extractHalosSeg_async(
    ...
    gpuError_t gpuErr = gpuDeviceSynchronize();
    if (gpuErr != gpuSuccess) {
        GpuError("siteComm.h: _extractHalosSeg_async gpuDeviceSynchronize failed:", gpuErr);
    }
