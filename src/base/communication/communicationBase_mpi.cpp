...
void CommunicationBase::initNodeComm() {
  ...
  ret = MPI_Comm_split_type(getCart_comm(), MPI_COMM_TYPE_SHARED, MyRank(),
                            mpi_info, &node_comm);
...
//! We can only check this reliably for openmpi. TODO: find out how to do it for
//! other mpi libraries
#if OPEN_MPI
...
/// Initialize MPI
void CommunicationBase::init(const LatticeDimensions &Dim,
                             const LatticeDimensions &Topo) {
...
void CommunicationBase::reduce(Matrix4x4Sym<float> *in, int nr) const {
  ...
  MPI_Allreduce(reinterpret_cast<float *>(in), reinterpret_cast<float *>(buf),
                nr * 10, MPI_FLOAT, MPI_SUM, cart_comm);
  for (int i = 0; i < nr; i++)
    in[i] = buf[i];
  delete[] buf;
}
...
void CommunicationBase::reduce(Matrix4x4Sym<double> *in, int nr) const {
  ...
  MPI_Allreduce(reinterpret_cast<double *>(in), reinterpret_cast<double *>(buf),
                nr * 10, MPI_DOUBLE, MPI_SUM, cart_comm);
  for (int i = 0; i < nr; i++)
    in[i] = buf[i];
  delete[] buf;
}
...
void CommunicationBase::reduce(GSU3<float> *in, int nr) const {
  ...
  MPI_Allreduce(reinterpret_cast<float *>(in), reinterpret_cast<float *>(buf),
                nr * 9, MPI_COMPLEX, MPI_SUM, cart_comm);
  for (int i = 0; i < nr; i++)
    in[i] = buf[i];
  delete[] buf;
}
...
void CommunicationBase::reduce(GSU3<double> *in, int nr) const {
  ...
  MPI_Allreduce(reinterpret_cast<double *>(in), reinterpret_cast<double *>(buf),
                nr * 9, MPI_DOUBLE_COMPLEX, MPI_SUM, cart_comm);
  for (int i = 0; i < nr; i++)
    in[i] = buf[i];
  delete[] buf;
}
...
std::complex<double>
CommunicationBase::globalAverage(std::complex<double> in) const {
  std::complex<double> recv;
  MPI_Allreduce(&in, &recv, 1, MPI_DOUBLE_COMPLEX, MPI_SUM, cart_comm);
  recv /= world_size;
  return recv;
...
std::complex<float>
CommunicationBase::globalAverage(std::complex<float> in) const {
  std::complex<float> recv;
  MPI_Allreduce(&in, &recv, 1, MPI_COMPLEX, MPI_SUM, cart_comm);
  recv /= (float)world_size;
  return recv;
...
int CommunicationBase::updateSegment(HaloSegment hseg, size_t direction,
  ...
  if (seg.getLength() != 0) {
    if (onDevice && info.p2p && useGpuP2P()) {
      ...
      IF(COMBASE_DEBUG)
      (stdLogger.debug("gpuMemcpyAsync: Copy ", seg.getLength(),
                       " bytes from rank ", MyRank(), " to rank ",
                       info.world_rank);)

          gpuErr =
              gpuMemcpyAsync(recvBase, sendBase, seg.getLength(),
                             gpuMemcpyDeviceToDevice, seg.getDeviceStream());
      if (gpuErr != gpuSuccess)
        GpuError("communicationBase_mpi.cpp: Failed to copy data "
                 "(DeviceToDevice) (1a)",
                 gpuErr);

    } else if (onDevice && info.sameRank &&
               (useGpuP2P() || gpuAwareMPIAvail())) {
      uint8_t *sendBase = seg.getMyDeviceSourcePtr();
      uint8_t *recvBase = seg.getMyDeviceDestinationPtr();

      IF(COMBASE_DEBUG)
      (stdLogger.debug("gpuMemcpyAsync (same rank): Copy ", seg.getLength(),
      ...
      IF(COMBASE_DEBUG)
      (stdLogger.debug("MPI_Isend (gpu): Send ", seg.getLength(),
                       " bytes from rank ", MyRank(), " to rank ",
                       info.world_rank);)

          MPI_Isend(sendBase, 1, seg.getMpiType(), rank, indexDest, cart_comm,
                    &seg.getRequestSend());
      MPI_Irecv(recvBase, 1, seg.getMpiType(), rank, index, cart_comm,
                &seg.getRequestRecv());
    } else {
      ...
      IF(COMBASE_DEBUG)
      (stdLogger.debug("MPI_Isend (cpu): Send ", seg.getLength(),
                       " bytes from rank ", MyRank(), " to rank ",
                       info.world_rank);)
          MPI_Isend(sendBase, 1, seg.getMpiType(), rank, indexDest, cart_comm,
                    &seg.getRequestSend());
...
// set MPI Barrier
void CommunicationBase::globalBarrier() const { MPI_Barrier(cart_comm); }

void CommunicationBase::nodeBarrier() const { MPI_Barrier(node_comm); }

void CommunicationBase::initIOBinary(std::string fileName, size_t filesize,
                                     size_t bytesPerSite, size_t displacement,
                                     LatticeDimensions globalLattice,
                                     LatticeDimensions localLattice,
                                     IO_Mode mode) {
  ...
  MPI_Type_create_subarray(4, globalLattice, localLattice, offset,
                           MPI_ORDER_FORTRAN, basetype, &fvtype);
  ...
  if (mode != READ)
    MPI_File_set_size(fh, filesize); // truncate if file exists and is too large
...
template int
CommunicationBase::updateSegment<true>(HaloSegment hseg, size_t direction,
                                       int leftRight,
                                       HaloOffsetInfo<true> &HalInfo);

template int
CommunicationBase::updateSegment<false>(HaloSegment hseg, size_t direction,
                                        int leftRight,
                                        HaloOffsetInfo<false> &HalInfo);

template void CommunicationBase::updateAll<true>(HaloOffsetInfo<true> &HalInfo,
                                                 unsigned int haltype);

template void
CommunicationBase::updateAll<false>(HaloOffsetInfo<false> &HalInfo,
                                    unsigned int haltype);
