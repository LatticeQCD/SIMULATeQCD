...
public:
        ...
        if (onDevice) {
            ...
            if (sequentialLoop) {
                ...
                for (size_t i = 0; i < NStacks; i++) {
                    ...
                    if (gpuErr) GpuError("LatticeContainer::reduceStackedLocal: hipcub::DeviceReduce::Sum (1)", gpuErr);
                    ...
                    if (gpuErr) GpuError("LatticeContainer::reduceStackedLocal: hipcub::DeviceReduce::Sum (2)", gpuErr);
                }
                ...
                if (gpuErr) GpuError("LatticeContainer::reduceStackedLocal: hipcub::DeviceSegmentedReduce::Sum (1)", gpuErr);
                ...
                if (gpuErr) GpuError("LatticeContainer::reduceStackedLocal: hipcub::DeviceSegmentedReduce::Sum (2)", gpuErr);
            }
    ...
    void reduce(elemType &value, size_t size, bool rootToAll = false) {
        ...
        if (onDevice) {
            ...
            if (gpuErr) GpuError("LatticeContainer::reduce: hipcub::DeviceReduce::Sum (1)", gpuErr);

            HelperArray->template adjustSize<void *>(temp_storage_bytes);

            gpuErr = CubReduce(HelperArray->getPointer(), &temp_storage_bytes, ContainerArray->template getPointer<elemType>(),
                            d_out->template getPointer<elemType>(), size);
            if (gpuErr) GpuError("LatticeContainer::reduce: hipcub::DeviceReduce::Sum (2)", gpuErr);
    ...
    void reduceMax(elemType &value, size_t size, bool rootToAll = false) {
        ...
        if (onDevice) {
            ...
            gpuError_t gpuErr = CubReduceMax(NULL, &temp_storage_bytes, ContainerArray->getPointer(),
                                    d_out->template getPointer<elemType>(), size);
            if (gpuErr) GpuError("LatticeContainer::reduceMax: hipcub::DeviceReduce::Max (1)", gpuErr);

            HelperArray->template adjustSize<void *>(temp_storage_bytes);

            gpuErr = CubReduceMax(HelperArray->getPointer(), &temp_storage_bytes, ContainerArray->getPointer(),
                             d_out->template getPointer<elemType>() , size);
            if (gpuErr) GpuError("LatticeContainer::reduceMax: hipcub::DeviceReduce::Max (2)", gpuErr);
            gpuErr = gpuMemcpy(&result, d_out->template getPointer<elemType>(), sizeof(result), gpuMemcpyDeviceToHost);
            if (gpuErr)
                GpuError("Reductionbase.h: Failed to copy data", gpuErr);
            // gpuFree(d_out);
