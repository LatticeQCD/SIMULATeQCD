 ...
 * (single value, fixed number of values, dynamic number of values) are then
 * derived from this (as templates, with the type and possibly the number of
 * values as template parameters). The idea is that the ParameterList only deals
 * with ParameterBase pointers, which point to actual (derived) Parameters.
 * Different types of parameters have different readstream() and print()
 * implementations, allowing the ParameterList to be generic when setting the
 * parameters.
 */
class ParameterBase {
protected:
  ...
  //! print value(s) to stream. This is needed because a friend function (like
  //! operator<<) can not be virtual.
...
template <class T, uint size = 1> class Parameter : public ParameterBase {
private:
  std::vector<T> values;
...
//! A parameter with a single value. This is a specification of the above for
//! size=1
template <class T> class Parameter<T, 1> : public ParameterBase {
private:
  T value;
...
template <class T> class DynamicParameter : public ParameterBase {
  std::vector<T> values;
...
public:
  ...
  // get vector
  std::vector<T> get() { return values; };

  //! read only access operator
  const T &operator[](int i) const { return values[i]; };

  //! backwards compatibility with old implementation #TODO: set a deprecated
  //! flag and see where the compiled complains
  const T &operator()(int i = 0) const { return values.at(i); }
};

//! explicit specialization of member function to read in multiple strings at
//! once that are separated by a space
template <>
...
private:
  bool readstream(std::istream &, int argc, char **argv,
...
public:
  ...
  void addOptional(ParameterBase &p, const std::string &name) {
    add(p, name, false);
  };
  ...
  void addDefault(Parameter<T, count> &p, const std::string &name,
                  const T *def) {
    addOptional(p, name);
    p.hasdefault = true;
   ...
   */
  bool readfile(const CommunicationBase &comm, const std::string &filename,
