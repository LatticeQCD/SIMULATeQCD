...
class HDF5FileWriter {
    ...
    public:
        ...
        HDF5FileWriter(CommunicationBase& commBase, LatticeParameters& latParams, const std::string& fileName) :
            ...
            group_name_corr("EMTU_Correlator"),
            ...
            dataSet_name_r2Counts("r2_counts"), dataSet_name_LL("G_LL") {
            
            TensorDecomposition<floatT, 0> tensor_decomposition(_commBase);
            ...
            r2max = tensor_decomposition.get_r2max();
            ...
            group_corr = new Group(group_gradFlowMeasurements->createGroup(group_name_corr));
            ...
            initDims_CorrQuantity[0] = 0;
            initDims_CorrQuantity[1] = r2max + 1;
            ...
            dataSpace_GLL = new DataSpace(2, initDims_CorrQuantity, maxDims_GLL);
            ...
            DSetCreatPropList propList_GLL;
            propList_GLL.setChunk(2, chunkSize_GLL);
            ...
            dataSet_flowTimeNecessary = new DataSet(group_corr->createDataSet(dataSet_name_flowTimeNecessary, *hdf5floatT, *dataSpace_flowTimeQuantity, propList_flowTimeQuantity));
            ...
            dataSet_r2Counts = new DataSet(group_corr->createDataSet(dataSet_name_r2Counts, PredType::NATIVE_INT, *dataSpace_r2Counts));
            dataSet_LL = new DataSet(group_corr->createDataSet(dataSet_name_LL, *compTypeComplex, *dataSpace_GLL, propList_GLL));
        ...
        void writeEMTUCorrData(
            const std::vector<COMPLEX(floatT)>& vec_EMTU_LL_COMPLEX
        ) {
            // create vector of ComplexData instead of COMPLEX(floatT)
            std::vector<ComplexData<floatT>> vec_EMTU_LL_complex_data(r2max+1);
            for (int r2 = 0; r2 < r2max + 1; r2++) {
                vec_EMTU_LL_complex_data[r2] = {real(vec_EMTU_LL_COMPLEX[r2]), imag(vec_EMTU_LL_COMPLEX[r2])};
            }

            // get dataspace of dataset
            DataSpace *filespace_LL = new DataSpace(dataSet_LL->getSpace());

            // get rank
            int rank = filespace_LL->getSimpleExtentNdims();

            // get current dimensions
            std::vector<hsize_t> currentDims(rank);
            filespace_LL->getSimpleExtentDims(currentDims.data(), NULL);

            // set offset, amount and new size
            hsize_t offset[2] = {currentDims[0], 0};
            hsize_t amount[2] = {1, currentDims[1]};
            hsize_t newsize[2] = {currentDims[0]+1, currentDims[1]};
            dataSet_LL->extend(newsize);
            
            // select hyperslab in file
            filespace_LL = new DataSpace(dataSet_LL->getSpace());
            filespace_LL->selectHyperslab(H5S_SELECT_SET, amount, offset);
            ...
            if (IamRoot()) {
                // write complex data
                dataSet_LL->write(vec_EMTU_LL_complex_data.data(), *compTypeComplex, *memoryspace, *filespace_LL);
            }
        }
