...
public :
    //! Copy constructor
     HOST LatticeDimensions(const LatticeDimensions &lhs) {
    ...
    //! Construct from (x,y,z,t)
     HOST LatticeDimensions(const int x, const int y, const int z, const int t) {
        c[0] = x;
        c[1] = y;
        c[2] = z;
        c[3] = t;
    ...
    //! Construct from int* (also works with Parameter<int,4>)
     HOST LatticeDimensions(const int *dim) {
        for (int i = 0; i < 4; i++)
            c[i] = dim[i];
    }

    //! Cast to int* (for usage in c-style MPI functions)
     HOST operator int *() { return c; }

    //! same with const
     HOST operator const int *() const { return c; };

    //! [] operator for member access
     HOST int &operator[](int mu) { return c[mu]; };

    //! const [] operator for r/o member access
     HOST const int &operator[](int mu) const { return c[mu]; };

    //! Component-wise multiplication, (x1*x2, y1*y2, z1*z2, t1*t2)
     HOST LatticeDimensions operator*(const LatticeDimensions lhs) const {
    ...
    //! modulo operation that returns coordinates within 0<=x<lx etc. (as long as the input is within -lx-1<x<2lx
     HOST LatticeDimensions operator%(const LatticeDimensions box) const {
    ...
    //! scalar multiplication
     HOST friend LatticeDimensions operator*(const int a, const LatticeDimensions &d) {
        return LatticeDimensions(a * d[0], a * d[1], a * d[2], a * d[3]);
    }

    //! Checks if all four dimension values are equal
     HOST bool operator==(const LatticeDimensions &lhs) const {
        for (int i = 0; i < 4; i++)
            if (c[i] != lhs.c[i])
                return false;
        return true;
    ...
    HOST void mv(const int mu, const bool plus) {
        if ((mu < 0) || (mu >= 4))
    ...
    //! Formatted (debug) output
     HOST friend std::ostream &operator<<(std::ostream &str,
    ...
    //! Return all four entries multiplied
     HOST long mult() const {
        long res = 1;
        for (int i = 0; i < 4; i++) res *= (long) c[i];
        return res;
    }

     HOST long summed() const {
        long res = 0;
        for (int i = 0; i < 4; i++) res += (long) c[i];
        return res;
    }
    ...
    //! Return if x,y,z,t are 0<=x<lx, 0<=y<ly ...
     HOST bool inLimit(int x, int y, int z, int t) const {
        return inLimit(LatticeDimensions(x, y, z, t));
    }

     HOST bool inLimit(const LatticeDimensions &in) const {
        for (int i = 0; i < 4; i++)
    ...
    //! this returns x + y*LX + z*LX*LY + t*LX*LY*LZ
     HOST size_t offset(const LatticeDimensions &in) const {
        size_t ret = in[0] + c[0] * in[1] + c[0] * c[1] * in[2] + c[0] * c[1] * c[2] * in[3];
        return ret;
    }

    //! Return the lowest entry
     HOST int lowest_value() const {
        int res = c[1];
        for (int i = 0; i < 4; i++)
            if (c[i] < res)res = c[i];
        return res;
    ...
    //! Return the lowest entry
    HOST int lowest_spatial_value() const {
        int res = c[1];
        for (int i = 0; i < 3; i++)
            if (c[i] < res)res = c[i];
        return res;
