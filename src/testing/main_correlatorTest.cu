...
int main(int argc, char *argv[]) { /// ---------------------------------------------------------------------- BEGIN MAIN
    ...
    try {
        ...
        timer.start();
        corrTools.correlateAt<GSU3<PREC>, GCOMPLEX(PREC), AxB<PREC>>("spacetime", CPUfield3, CPUfield4, CPUnorm,
                                                                     CPUcorrComplex1, true);
        timer.stop();
        ...
        timer.start();
        corrTools.correlateAt<GSU3<PREC>, GCOMPLEX(PREC), AxB<PREC>>("spatial", CPUSfield3, CPUSfield4, CPUSnorm,
                                                                     CPUScorrComplex, true);
        timer.stop();
        ...
        if (HAVECONFIG) {
            ...
            /// GPU tr A x Bt
            timer.start();
            corrTools.correlateAt<GSU3<PREC>, GCOMPLEX(PREC), trAxBt<PREC>>("spacetime", CPUfield3, CPUfield4, CPUnorm,
                                                                            CPUcorrComplex2);
            timer.stop();
            ...
            for (int ir2 = 0; ir2 < corrTools.UAr2max + 1; ir2++) {
                ...
                if (ratnorm > 0.) {
                    ...
                    if (!(corrComplex == corrComplexControl)) {
                        rootLogger.error("UA tr A x B: ir2, tr(test), tr(control) = ", ir2, ", ", corrComplex, ", ",
                                         corrComplexControl);
                        lerror = true;
                    }
                    ...
                    if (!(corrComplex == corrComplexControl)) {
                        rootLogger.error("UA tr A x Bt: ir2, tr(test), tr(control) = ", ir2, ", ", corrComplex, ", ",
                                         corrComplexControl);
                        lerror = true;
                    }
            ...
            for (int ir2 = 0; ir2 < corrTools.USr2max + 1; ir2++) {
                ...
                if (ratnorm > 0.) {
                    ...
                    if (!(corrComplex == corrComplexControl)) {
                        rootLogger.error("US gauge x gauge: ir2, tr(test), tr(control) = ", ir2, ", ", corrComplex,
                                         ", ", corrComplexControl);
                        lerror = true;
                    }
                }
        ...
        timer.start();
        corrTools.correlateAt<GSU3<PREC>, GCOMPLEX(PREC), trAxtrBt<PREC>>("spatial", thermalWilsonLine,
                                                                          thermalWilsonLine, CPUSnorm, PLoopBareSusc);
        timer.stop();
