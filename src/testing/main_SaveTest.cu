...
int main(int argc, char *argv[]){
    try {
        ...
        if (abs(plaq - plaq_test) > epsilon) {
            ...
            rootLogger.info(CoutColors::green, "writeconf_nersc succeeded: Computed plaquettes are equal",
                            CoutColors::reset);
        }
        ...
        if (abs(clov - clov_test) > epsilon) {
            ...
            rootLogger.info(CoutColors::green, "writeconf_nersc succeeded: Computed clovers are equal ",
                            CoutColors::reset);
        }
