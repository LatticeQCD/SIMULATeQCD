                ...
                for (int pt=0; pt<GInd::getLatData().globLT; pt+= GInd::getLatData().globLT/(param.sources()[3])){
	             ...
	             spinor_in = spinor_out;
                    // tr_spinorXspinor(spinor_in,spinor_out);
                     ...
                     spinor_host12 = spinor_in;
                     COMPLEX(double) sum_b[12*12];
		     COMPLEX(double) sum_a[12*12];
		     COMPLEX(double) vec_b[12*12];
		     COMPLEX(double) vec_a[12*12];

                     //beginning of test. First caluculate sum of spinor_in and value at  0, sum divided by 1/sqrt(volume) , do to chosen normalization of fourier transformation
                     ...
                     for(int spincolor1 =0; spincolor1 < 12; spincolor1 ++){
                         for(int spincolor2 =0; spincolor2 < 12; spincolor2 ++){
                              ...
                              temp = sumXYZT(spinor_in,redBaseDevice, spincolor1, spincolor2)/sqrt(1.0*GInd::getLatData().globLX*GInd::getLatData().globLY*GInd::getLatData().globLZ*GInd::getLatData().globLT);
                              //printf("%d %d before sum %.16f + im* %.16f \n", spincolor1, spincolor2, temp.cREAL , temp.cIMAG );
                              sum_b[spincolor1+12*spincolor2] = temp;


			      gSiteStack pSite = GInd::getSiteStack(GInd::getSite(0,0,0,0),spincolor2);
			      Vect12<PREC> tmp0 = spinor_host12.getAccessor().template getElement<PREC>(pSite);
			      temp = tmp0.data[spincolor1];
			      //printf("%d %d before Vec %.16f + im* %.16f \n", spincolor1, spincolor2, temp.cREAL , temp.cIMAG );
                              vec_b[spincolor1+12*spincolor2] = temp;
                          }
                     }



		     fourierClass.performFourier4DSpinor1212<HaloDepth>(spinor_in,spinor_in,redBaseDevice,redBaseHost,-1,12);


                     //Caluculate sum of fourier transormation and value at  0
		     ...
		     spinor_host12 = spinor_in;
                     for(int spincolor1 =0; spincolor1 < 12; spincolor1 ++){
                         for(int spincolor2 =0; spincolor2 < 12; spincolor2 ++){
                              ...
                              COMPLEX(double) temp(0.0,0.0);
                              temp = sumXYZT(spinor_in,redBaseDevice, spincolor1, spincolor2)/sqrt(1.0*GInd::getLatData().globLX*GInd::getLatData().globLY*GInd::getLatData().globLZ*GInd::getLatData().globLT);
                              //printf("%d %d after sum %.16f + im* %.16f \n", spincolor1, spincolor2, temp.cREAL , temp.cIMAG );
                              sum_a[spincolor1+12*spincolor2] = temp;

			      gSiteStack pSite = GInd::getSiteStack(GInd::getSite(0,0,0,0),spincolor2);
                              Vect12<PREC> tmp0 = spinor_host12.getAccessor().template getElement<PREC>(pSite);
                              temp = tmp0.data[spincolor1];
                              //printf("%d %d after Vec %.16f + im* %.16f \n", spincolor1, spincolor2, temp.cREAL , temp.cIMAG );
                              vec_a[spincolor1+12*spincolor2] = temp;

                          }
                     }

                     // up to normalization first sum should equal fourier at 0 and firsst value at zero should be sum of fourier

                     gSite site = GInd::getSite(0,0,0,0);
		     sitexyzt coord = GIndexer<All, 2>::getLatData().globalPos(site.coord);
                     if(coord[0] == 0 && coord[1] == 0 && coord[2] == 0 && coord[3] == 0 ){

		     for(int spincolor1 =0; spincolor1 < 12; spincolor1 ++){
                         for(int spincolor2 =0; spincolor2 < 12; spincolor2 ++){
			      COMPLEX(double) temp(0.0,0.0);
			      temp = sum_b[spincolor1+12*spincolor2] - vec_a[spincolor1+12*spincolor2];
			      printf("%d %d diff 1 %.16f + im* %.16f \n", spincolor1, spincolor2, temp.cREAL , temp.cIMAG );

                              temp = sum_a[spincolor1+12*spincolor2] - vec_b[spincolor1+12*spincolor2];
                              printf("%d %d diff 2 %.16f + im* %.16f \n", spincolor1, spincolor2, temp.cREAL , temp.cIMAG );

                              printf("%d %d values_ba %.16f + im* %.16f     %.16f + im* %.16f  \n", spincolor1, spincolor2, sum_b[spincolor1+12*spincolor2].cREAL , sum_b[spincolor1+12*spincolor2].cIMAG,vec_a[spincolor1+12*spincolor2].cREAL , vec_a[spincolor1+12*spincolor2].cIMAG );         
                              printf("%d %d values_ab %.16f + im* %.16f     %.16f + im* %.16f  \n", spincolor1, spincolor2, sum_a[spincolor1+12*spincolor2].cREAL , sum_a[spincolor1+12*spincolor2].cIMAG,vec_b[spincolor1+12*spincolor2].cREAL , vec_b[spincolor1+12*spincolor2].cIMAG );
