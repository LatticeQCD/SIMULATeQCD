/*
 * MDWF operator workspace smoke test.
 *
 * This test verifies that MDWFOperatorWorkspace owns and reuses the temporary
 * fields needed by the MDWF operator wrappers.  It compares the workspace
 * clover application against the explicit temporary-field call.  It does not
 * call CG, RHMC/HMC, force code, or gauge-link smearing.
 */

#include "../simulateqcd.h"
#include "../experimental/mdwf/MDWFOperator.h"

#include <cmath>

template<class floatT, Layout LatLayout, size_t HaloDepth, size_t Ls>
struct FillMDWFWorkspacePattern {
    __host__ __device__ Vect12<floatT> operator()(gSiteStack site) {
        Vect12<floatT> out(0.0);

        for (size_t component = 0; component < 12; component++) {
            out.data[component] = COMPLEX(floatT)(
                static_cast<floatT>(1000 * site.stack + site.isite + component + 1), 0.0);
        }
        return out;
    }
};

template<size_t Ls>
void runMDWFOperatorWorkspaceSmokeTest(CommunicationBase &commBase) {
    const size_t HaloDepth = 2;
    typedef GIndexer<All, HaloDepth> GInd;

    Gaugefield<double, true, HaloDepth, R18> gauge(commBase, "MDWF_workspace_gauge");
    grnd_state<false> h_rand;
    grnd_state<true> d_rand;
    h_rand.make_rng_state(1337);
    d_rand = h_rand;
    gauge.random(d_rand.state);

    MDWFSpinor<double, true, All, HaloDepth, Ls> spinorIn(commBase, "MDWF_workspace_in");
    MDWFSpinor<double, true, All, HaloDepth, Ls> workspaceOut(commBase, "MDWF_workspace_out");
    MDWFSpinor<double, true, All, HaloDepth, Ls> referenceOut(commBase, "MDWF_workspace_reference_out");
    MDWFSpinor<double, true, All, HaloDepth, Ls> wilsonPart(commBase, "MDWF_workspace_reference_wilson_part");
    MDWFSpinor<double, true, All, HaloDepth, Ls> fifthPart(commBase, "MDWF_workspace_reference_fifth_part");
    MDWFSpinor<double, true, All, HaloDepth, Ls> wilsonTmp(commBase, "MDWF_workspace_reference_wilson_tmp");
    MDWFSpinor<double, false, All, HaloDepth, Ls> workspaceHost(commBase, "MDWF_workspace_host");
    MDWFSpinor<double, false, All, HaloDepth, Ls> referenceHost(commBase, "MDWF_workspace_reference_host");

    Spinorfield<double, true, All, HaloDepth, 18, 1> fmunuUpper(commBase, "MDWF_workspace_reference_fmunu_upper");
    Spinorfield<double, true, All, HaloDepth, 18, 1> fmunuLower(commBase, "MDWF_workspace_reference_fmunu_lower");
    Spinorfield<double, true, All, HaloDepth, 18, 1> fmunuInvUpper(commBase, "MDWF_workspace_reference_fmunu_inv_upper");
    Spinorfield<double, true, All, HaloDepth, 18, 1> fmunuInvLower(commBase, "MDWF_workspace_reference_fmunu_inv_lower");

    spinorIn.template iterateOverBulk<>(FillMDWFWorkspacePattern<double, All, HaloDepth, Ls>());
    spinorIn.updateAll();

    MDWFFifthDimCoefficients<double> coeff(2.0, 3.0, 5.0, 7.0, 11.0);
    MDWFOperatorWorkspace<double, HaloDepth, HaloDepth, Ls> workspace(gauge, "MDWF_workspace_internal");

    workspace.applyClover(workspaceOut, spinorIn, coeff, 1.0, 0.5, true);
    applyMDWFCloverOperator<double, HaloDepth, HaloDepth, Ls>(
        referenceOut, gauge, wilsonPart, fifthPart, wilsonTmp, fmunuUpper, fmunuLower,
        fmunuInvUpper, fmunuInvLower, spinorIn, coeff, 1.0, 0.5, true);

    workspaceHost = workspaceOut;
    referenceHost = referenceOut;

    Vect12ArrayAcc<double> workspaceAcc = workspaceHost.getAccessor();
    Vect12ArrayAcc<double> referenceAcc = referenceHost.getAccessor();

    double maxDiff = 0.0;
    for (size_t isite = 0; isite < GInd::getLatData().vol4; isite++)
        for (size_t stack = 0; stack < Ls; stack++) {
            const gSiteStack site = GInd::getSiteStack(GInd::getSite(isite), stack);
            Vect12<double> workspaceValue = workspaceAcc.getElement(site);
            Vect12<double> referenceValue = referenceAcc.getElement(site);
            for (size_t component = 0; component < 12; component++) {
                const double diff = std::abs(real(workspaceValue.data[component] - referenceValue.data[component]))
                                    + std::abs(imag(workspaceValue.data[component] - referenceValue.data[component]));
                if (diff > maxDiff) {
                    maxDiff = diff;
                }
            }
        }

    if (maxDiff > 1e-12) {
        throw std::runtime_error(stdLogger.fatal(
            "MDWF operator workspace smoke test failed with maxDiff = ", maxDiff));
    }

    rootLogger.info("MDWF operator workspace smoke test passed with Ls = ", Ls);
}

int main(int argc, char **argv) {
    try {
        stdLogger.setVerbosity(INFO);

        LatticeParameters param;
        CommunicationBase commBase(&argc, &argv, true);
        param.readfile(commBase, "../parameter/tests/mdwfFifthDimTest.param", argc, argv);
        commBase.init(param.nodeDim());

        const int HaloDepth = 2;
        initIndexer(HaloDepth, param, commBase);

        runMDWFOperatorWorkspaceSmokeTest<8>(commBase);
        return 0;
    }
    catch (const std::runtime_error &error) {
        rootLogger.error("There has been a runtime error!");
        rootLogger.error(error.what());
        return -1;
    }
}
