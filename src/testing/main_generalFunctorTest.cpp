...
struct SimplePlaq {
    ...
    SU3Accessor<floatT> gAcc;     //! Here we just need the accessor from the gaugefield reference and nothing else
...
struct SillyMultiplicationInsideMuLoop{
    ...
    __host__ __device__ void initialize(__attribute__((unused)) gSite site){
        ...
        my_mat = su3_one<floatT>();
    }

    //! This is called inside the mu loop. We're just doing some silly multiplications here.
    __host__ __device__ SU3<floatT> operator()(__attribute__((unused)) gSite site, size_t mu) {
...
void run_func(CommunicationBase &commBase) {
     ...
     * return the result that we want to store in our class, e.g. a Vect3<floatT> if you work on a Spinor object.
     *
    ...
    gauge.iterateWithConst(su3_one<floatT>());
     ...
     *   gauge.iterateOverFullAllMu(GeneralOperator<double, SU3<floatT>, mult>(2.0, gauge))
     ...
     * gauge/2, because operator SU3<floatT> / int is not defined.
     */
    ...
    gauge = gauge * 0.5 + su3_one<floatT>();
    plaq_ref = 16.0;
    plaq = compute_plaquette(gauge, latContainer);
    ...
    gauge.one();
    gauge.template iterateOverBulkAtMu<1>(COMPLEX(floatT)(2) * gauge);
    ...
    //!   SU3Accessor.setElement(calcWriteIndex(site), returnLink(site));
    ...
    COMPLEX(floatT) res_projected;

    //! For a reference which we use for testing.
    COMPLEX(floatT) ref_projected;

    LatticeContainer<true,COMPLEX(floatT)> redBase2(commBase);
    ...
    spinor2.iterateWithConst(vect3_one<floatT>());
