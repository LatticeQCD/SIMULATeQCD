...
struct FillStacks{
    ...
    __host__ __device__ Vect3<floatT> operator()(gSiteStack site){
        Vect3<floatT> ret = static_cast<floatT>(site.stack + 1) * vect3_one<floatT>();
        return ret;
    }
};
...
void run_func(CommunicationBase &commBase) {
    ...
    std::vector<COMPLEX(floatT)> ref_projected(NStacks);

    COMPLEX(floatT) reference;
    if (LatLayout == All) {
    ...
    for (size_t i = 0; i < NStacks; i++){
        spinorArray1[i].iterateWithConst(vect3_one<floatT>());
    ...
    stackedSpinor1.iterateWithConst(vect3_one<floatT>());
