...
struct wilsonParam : LatticeParameters {
    ...
    wilsonParam() {
        ...
        addDefault (gtolerance,"gtolerance",floatT(1e-6));
...
/// spinors after flow to save on maximum memory used
    ...
    Spinorfield<PREC, false, All, HaloDepth, 12, 12> ** spinor_store;

    spinor_out = new Spinorfield<PREC, true, All, HaloDepth, 12, 12>*[1];
    for (int i = 0; i < 1; i++) {
       spinor_out[i] = new Spinorfield<PREC, true, All, HaloDepth, 12, 12>(commBase); 
    }

    spinor_store = new Spinorfield<PREC, false, All, HaloDepth, 12, 12>*[nWave-1];
    for (int i = 0; i < nWave-1; i++) {
       spinor_store[i] = new Spinorfield<PREC, false, All, HaloDepth, 12, 12>(commBase);
    }
...
/////old version of loading sources
  /* 
...
*/
    ...
    COMPLEX(PREC) CC_g5g4[lt*nWave*nWave];

    COMPLEX(PREC) CC_g12[lt*nWave*nWave];
    COMPLEX(PREC) CC_g13[lt*nWave*nWave];
    COMPLEX(PREC) CC_g14[lt*nWave*nWave];
    COMPLEX(PREC) CC_g23[lt*nWave*nWave];
    COMPLEX(PREC) CC_g24[lt*nWave*nWave];
    COMPLEX(PREC) CC_g34[lt*nWave*nWave];
   ...
   //FourierClass<PREC> fourierClass(commBase);
            ...
            for (int pz=0; pz<GInd::getLatData().globLZ; pz+= GInd::getLatData().globLZ/(param.sources()[2])){
                for (int pt=0; pt<GInd::getLatData().globLT; pt+= GInd::getLatData().globLT/(param.sources()[3])){
                     ...
                     for(int ss = nWave-1; ss > -1; ss --){
                       ...
                       _dslashinverseSC4.correlator(spinor_out[0][0],spinor_in,maxiter,tolerance);
                       _dslashinverseSC4.antiperiodicBoundaries();
			   ...
			   spinor_store[ss-1][0] = spinor_out[0][0];
                     ...
                     if(ss==0){
                           spinor_in = spinor_out[0][0];
                       }
                       else{
                           spinor_in = spinor_store[ss-1][0];
                       }
		       ...
		       for(int ss2 = 0; ss2 < nWave; ss2 ++){
                     ...
                     for (int t=0; t<GInd::getLatData().globLT; t++){
                         CC_g5[ss+nWave*(ss2+nWave*t)] += sumXYZ_TrMdaggerMwave((int)((t+pos[3])%(lt)),spinor_out[0][0],spinor_in,spinor_device,redBaseDevice,2*ss2+1,0,2);

                     }

                     // tr( (g5*M^d*g5) g5g4 M g4g5 )
                     //spinor_in = spinor_out[1][0];

                     source.gammaMuRight<PREC,All,HaloDepth,3>(spinor_in);
                     source.gammaMu<PREC,All,HaloDepth,12,3>(spinor_in);

                     for (int t=0; t<GInd::getLatData().globLT; t++){
                         CC_g5g4[ss+nWave*(ss2+nWave*t)] += sumXYZ_TrMdaggerMwave((int)((t+pos[3])%(lt)),spinor_out[0][0],spinor_in,spinor_device,redBaseDevice,2*ss2+1,0,2);
                     }
		     ...
		     for (int t=0; t<GInd::getLatData().globLT; t++){
                         CC_g12[ss+nWave*(ss2+nWave*t)] += sumXYZ_TrMdaggerMwave((int)((t+pos[3])%(lt)),spinor_out[0][0],spinor_in,spinor_device,redBaseDevice,2*ss2+1,0,2);
                     }
		     ...
		     source.gammaMuRight<PREC,All,HaloDepth,1>(spinor_in);
                     ...
                     for (int t=0; t<GInd::getLatData().globLT; t++){
                         CC_g13[ss+nWave*(ss2+nWave*t)] += sumXYZ_TrMdaggerMwave((int)((t+pos[3])%(lt)),spinor_out[0][0],spinor_in,spinor_device,redBaseDevice,2*ss2+1,0,2);
                     }
                     ...
                     for (int t=0; t<GInd::getLatData().globLT; t++){
                         CC_g14[ss+nWave*(ss2+nWave*t)] += sumXYZ_TrMdaggerMwave((int)((t+pos[3])%(lt)),spinor_out[0][0],spinor_in,spinor_device,redBaseDevice,2*ss2+1,0,2);
                     }
                     ...
                     for (int t=0; t<GInd::getLatData().globLT; t++){
                         CC_g23[ss+nWave*(ss2+nWave*t)] += sumXYZ_TrMdaggerMwave((int)((t+pos[3])%(lt)),spinor_out[0][0],spinor_in,spinor_device,redBaseDevice,2*ss2+1,0,2);
                     }
                     ...
                     for (int t=0; t<GInd::getLatData().globLT; t++){
                         CC_g24[ss+nWave*(ss2+nWave*t)] += sumXYZ_TrMdaggerMwave((int)((t+pos[3])%(lt)),spinor_out[0][0],spinor_in,spinor_device,redBaseDevice,2*ss2+1,0,2);
                     }
                     ...
                     for (int t=0; t<GInd::getLatData().globLT; t++){
                         CC_g34[ss+nWave*(ss2+nWave*t)] += sumXYZ_TrMdaggerMwave((int)((t+pos[3])%(lt)),spinor_out[0][0],spinor_in,spinor_device,redBaseDevice,2*ss2+1,0,2);
                     }
    ...
    fileOut << "states " << " " << "real(I)"     << " " << "real(I)"     <<
			    ...
			    " " << "real(g1 g3)" << " " << "real(g1 g3)" <<
			    " " << "real(g1 g4)" << " " << "real(g1 g4)" <<
			    " " << "real(g2 g3)" << " " << "real(g2 g3)" <<
			    " " << "real(g2 g4)" << " " << "real(g2 g4)" <<
			    " " << "real(g3 g4)" << " " << "real(g3 g4)" <<   "\n";
    ...
    for(int ss = 0; ss < nWave; ss ++){
       ...
       for (int t=0; t<lt; t++){   
        fileOut << t << " " << ss << " " << ss2 << 
		        ...
		        " " << norm*real(CC_g5g4[ss+nWave*(ss2+nWave*t)]) << " " << norm*imag(CC_g5g4[ss+nWave*(ss2+nWave*t)]) <<
		        " " << norm*real(CC_g12[ss+nWave*(ss2+nWave*t)])  << " " << norm*imag(CC_g12[ss+nWave*(ss2+nWave*t)])  <<
                        " " << norm*real(CC_g13[ss+nWave*(ss2+nWave*t)])  << " " << norm*imag(CC_g13[ss+nWave*(ss2+nWave*t)])  <<
			" " << norm*real(CC_g14[ss+nWave*(ss2+nWave*t)])  << " " << norm*imag(CC_g14[ss+nWave*(ss2+nWave*t)])  <<
			" " << norm*real(CC_g23[ss+nWave*(ss2+nWave*t)])  << " " << norm*imag(CC_g23[ss+nWave*(ss2+nWave*t)])  <<
			" " << norm*real(CC_g24[ss+nWave*(ss2+nWave*t)])  << " " << norm*imag(CC_g24[ss+nWave*(ss2+nWave*t)])  <<
			" " << norm*real(CC_g34[ss+nWave*(ss2+nWave*t)])  << " " << norm*imag(CC_g34[ss+nWave*(ss2+nWave*t)])  << "\n";
    }}}
