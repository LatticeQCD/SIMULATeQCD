...
int main(int argc, char *argv[]) {
    ...
    try {
        ...
        corrTools.correlateAt<GSU3<PREC>, GCOMPLEX(PREC), trAxtrBt<PREC>>("spatial", thermalWilsonLine,
                                                                          thermalWilsonLine, CPUnorm, ABareSusc);
        /// Here we have A and B the same operator, so XYswapSymmetry == True here.
        corrTools.correlateAt<GSU3<PREC>, PREC, trReAxtrReB<PREC>>("spatial", thermalWilsonLine, thermalWilsonLine,
                                                                   CPUnorm, LBareSusc, true);
        corrTools.correlateAt<GSU3<PREC>, PREC, trImAxtrImB<PREC>>("spatial", thermalWilsonLine, thermalWilsonLine,
                                                                   CPUnorm, TBareSusc, true);
        timer.stop();
