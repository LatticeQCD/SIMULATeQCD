    ...
    try {
        ...
        if (lp.EnergyMomentumTensorCorr() && lp.sublattice_lt() < 4) {
            throw std::runtime_error(
                    stdLogger.fatal("Temporal extent of sub lattices can NOT be smaller than 4 when measure energy-"
                                    "momentum tensor correlators!!!"));
        }

        if (lp.ColorElectricCorr() && lp.sublattice_lt() < 3) {
            throw std::runtime_error(
                    stdLogger.fatal("Temporal extent of sub lattices can NOT be smaller than 3 when measure "
        ...
        if (lp.latDim()[3] % lp.sublattice_lt() != 0) {
            throw std::runtime_error(
                    stdLogger.fatal("Temporal extent of sub lattices must be divisible full temporal extention !!!"));
        }
        ...
        size_t node_multiplication =
                lp.nodeDim()[0] * lp.nodeDim()[1] * lp.nodeDim()[2]; //the multiplication of spatial node dim
        size_t elem1 = global_spatial_vol / node_multiplication;//local spatial vol on each gpu(the size of sub ced1)
        size_t elem2 = elem1 - (lp.latDim()[0] / lp.nodeDim()[0] - 2) * (lp.latDim()[1] / lp.nodeDim()[1] - 2) *
                               (lp.latDim()[2] / lp.nodeDim()[2] - 2);//the size of sub cec2
        size_t elem3 = 1;
        ...
        if (MemAll + 20 > TotalDevice / 1024. / 1024.) { //20 is an estimation for miscellaneous memory comsumption
            throw std::runtime_error(
                    stdLogger.fatal("Device Memory[MB]:  ", TotalDevice / 1024. / 1024., "    Required Memory[MB]:  ",
                                    MemAll + 20,
        ...
        std::vector<Matrix4x4Sym<PREC>> SubShear_Nt_real((lp.PzMax() + 1) * lp.latDim()[3] * (lp.sublattice_lt() - 3),
                                                         0);
        std::vector<Matrix4x4Sym<PREC>> SubShear_Nt_imag((lp.PzMax() + 1) * lp.latDim()[3] * (lp.sublattice_lt() - 3),
                                                         0);
        ...
        for (int local_pos_t = 0; local_pos_t < lp.sublattice_lt(); local_pos_t++) {
            ...
            rootLogger.info("measure. update the sub-lattice at shift position: ", local_pos_t, "/",
                            lp.sublattice_lt());
            ...
            for (int i = 0; i < lp.num_meas(); ++i) {
                ...
                for (int sub_i = 0; sub_i < lp.latDim()[3] / lp.sublattice_lt(); sub_i++) {
                    ...
                    if (lp.EnergyMomentumTensorCorr()) {
                        sublatmeas.updateSubNorm(local_pos_t + sub_i * lp.sublattice_lt(), SubTbarbp00, SubSbp,
                                                 SubTbarbc00_SubSbc);
                        for (int dist = 0; dist < lp.sublattice_lt() -
                                                  3; dist++) { //minus 3 because T is from F, clover boundary can NOT hit sublattice boudndary
                            for (int pz = 0; pz <= lp.PzMax(); pz++) {
                                sublatmeas.updateSubEMT(local_pos_t + sub_i * lp.sublattice_lt(), lp.num_meas(),
                                                        sub_E_gpu, sub_U_gpu, SubBulk_Nt_p0, SubShear_Nt_p0,
                                                        SubBulk_Nt_real,
                    ...
                    if (lp.ColorElectricCorr()) {
                        sublatmeas.updateSubPolyCorr(local_pos_t + sub_i * lp.sublattice_lt(), lp.num_meas(),
                                                     sub_poly_Nt, sub1_cec_Nt, sub2_cec_Nt);
                    }
                }
            }
        }
        ...
        if (lp.EnergyMomentumTensorCorr()) {
            ...
            newTag_normEMT << std::scientific << std::setprecision(15) << Tbarbp00 * 2. << 2. * (18. - Sbp) << Tbarbc00
                           << Sbc << "\n";
            ...
            for (int pz = 0; pz <= lp.PzMax(); pz++) {
                contraction.ImproveContractionBulk(SubBulk_Nt_real, SubBulk_Nt_imag, lp.min_dist(), global_spatial_vol,
                                                   pz, ImproveBulk_Correlator);
            }
            ...
            datNameEMT_ImproveTraceAnomaly << lp.out_dir() << "MultiLevel_TraceAnomaly" << "_PzMax" << lp.PzMax()
                                           << lp.fileExt();
            ...
            //contraction for the improve emt correlators in bulk channel
            rootLogger.info(
                    "contration to obtain the multi-level improved energy-momentum correlator in shear channel");
            std::vector<PREC> ImproveShear_Correlator((lp.PzMax() + 1) * lp.latDim()[3], 0);
            for (int pz = 0; pz <= lp.PzMax(); pz++) {
                contraction.ImproveContractionShear(SubShear_Nt_real, SubShear_Nt_imag, lp.min_dist(),
                                                    global_spatial_vol, pz, ImproveShear_Correlator);
            }
        ...
        if (lp.ColorElectricCorr()) {
            ...
            newTagImprovePoly << std::scientific << std::setprecision(15) << real(ImprovePolyakovLoop) << " "
                              << imag(ImprovePolyakovLoop) << "\n";
            ...
            for (int i = lp.min_dist() + 3; i < lp.latDim()[3] / 2 + 1; i++) {
                newTagImproveColEleCorr << i << " " << std::scientific << std::setprecision(15)
                                        << real(ImproveColorElectricCorrelator[i]) << " "
                                        << imag(ImproveColorElectricCorrelator[i]) << "\n";
            }
        }
