...
///////////// gauge fixing
        ...
        PREC *results;
        results = new PREC[(GInd::getLatData().globvol3 / 2 + GInd::getLatData().globLX * GInd::getLatData().globLY) *
                           GInd::getLatData().globLT];
        ///
        timer.start();
...
/////////////// final check
        ...
        PREC sum = 0.0;
        for (size_t i = 0; i <
                           (GInd::getLatData().globvol3 / 2 + GInd::getLatData().globLX * GInd::getLatData().globLY) *
                           GInd::getLatData().globLT; i += 1) {
            sum += abs(results[i]);
        }
