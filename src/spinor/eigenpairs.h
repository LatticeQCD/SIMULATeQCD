...
private:
    ...
    eigenpairs(const eigenpairs<floatT, onDevice, LatticeLayout, HaloDepthSpin, NStacks> &) = delete;
    const eigenpairsParameters param;
...
public:
    ...
    void read_evnersc(const std::string &fname);
...
template<class floatT, bool onDevice, Layout LatticeLayout, size_t HaloDepthSpin, size_t NStacks>
inline Vect3arrayAcc<floatT> eigenpairs<floatT, onDevice, LatticeLayout, HaloDepthSpin, NStacks>::getAccessor() const {
    return (_spinor_lattice.getAccessor());
}

template<class floatT, bool onDevice, Layout LatLayout, size_t HaloDepthSpin, size_t Nstacks>
struct returnEigen {
    Vect3arrayAcc<floatT> _gAcc;

    explicit returnEigen(const eigenpairs<floatT, onDevice, LatLayout, HaloDepthSpin, Nstacks> &spinorIn);
