...
e.g. when you want to split your lattice in 8 parts, by e.g. setting `Nodes = 1 1 4 2` you can `Topology = 0 0 1 4` to get a "good" mapping
of MPI ranks to deviceRanks.

or if you want to spilt by setting `Nodes = 2 2 2 1` the optimal `Topology = 1 2 4 0`. With this topology setting you distribute the ranks over
the corners of a cube, just like the *nvlink* Topology on the Bielefeld cluster.

If you want to split the lattice in 6 parts by setting `Nodes = 1 1 2 3` you can use the `Topology = 0 0 4 1` to distribute 3 Ranks on 3 GPUs on the
top face and three ranks on the 3 GPUs on the bottom face of cube spanned by the *nvlink* topology on the Bielefeld cluster.

On summit, where you only have 6 GPUs per node and 3 GPUs per socket, you would choose
`Nodes = 1 1 2 3` you can use the `Topology = 0 0 3 1`.
