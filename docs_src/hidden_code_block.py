...
def visit_hcb_html(self, node):
    ...
    divheader = ("""<a href="javascript:showhide(document.getElementById('{divname}')); showhide(document.getElementById('{divname}_whitespace'))">"""
                 """{label}</a><br />"""
                 '''<div id="{divname}" style="display: {startdisplay}">'''
                 ).format(**fill_header)

    code_block = js_showhide + divheader + code_block \
                + """</div><div id="%s_whitespace" style="display: %s"><br></div>""" % (fill_header['divname'], 'block' if node['starthidden'] else 'none')
