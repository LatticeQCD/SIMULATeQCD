...
add_SIMULATeQCD_executable(<exec_name> src/<type_of_application>/main_<exec_name>.cu
...
Add the `HaloDepth` and precision which you want to use in you application in `SIMULATeQCD_target_compile_definitions(...)`. Available definitions are written in `define.h`.
