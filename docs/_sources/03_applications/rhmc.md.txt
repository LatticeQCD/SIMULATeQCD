...
algorithm is a way of updating gauge fields when simulating dynamical staggered fermions.
In particular, the system's Hamiltonian is considered as a function of MC time $\tau$.
It then evolves according to Hamilton's equations of motion, and 
[integrating](../05_modules/integrator.md) this
differential equation along $\tau$ is a strategy to suggest a new configuration.
This is called _molecular dynamics_ (MD), and its progress is measured in _trajectories_,
i.e. in the number of integrations. After a trajectory, the configuration is 
accepted/rejected with a standard Metropolis step, which
is an example of a _hybrid_ Monte Carlo algorithm. When working with staggered fermions,
one generally removes the four-fold degeneracy of fermion d.o.f. by taking the fourth
root of the Dirac operator. In practice, this fourth root is implemented as a rational
approximation. Using a rational approximation for the Hamiltonian of the MD is what
is meant by RHMC. A thorough overview of HMC can be found in, e.g.
[Gattringer and Lang](https://doi.org/10.1007/978-3-642-01850-3). A scheme for
doing HMC with staggered fermions was first made explicit
[here](https://doi.org/10.1103/PhysRevD.35.2531).

The trajectory is pushed by a fictitious driving force. There come contributions from
...
$
where $\phi$ is a pseudo-fermion field, and $\partial$ indicates a
...
You can learn more about the force [here](../05_modules/HISQforce.md).
