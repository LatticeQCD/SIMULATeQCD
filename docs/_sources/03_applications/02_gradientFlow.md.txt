...
necessary_flow_times=0.25 0.5                        # Set the flow-times which shouldn't be skipped by the fixed or adaptive stepsize 

ignore_fixed_startstepsize = 0                       # ignore the fixed stepsize and infer stepsizes from necessary_flow_times
...
GaugeFixNMax = 9000 #...or stop after a fixed number of steps; this way the program doesn't get stuck.
...
writter in one (ASCII) output file. `cloverTimeSlices`,
...
(e.g., Lüscher's papers).\
Remember temperature $T=\frac{1}{aN_\tau}$.
...
small intial stepsize and high accuracy, and saw that our correlation
...
-   This means that for a fixed number of intermediate flow times that
    we want to expicitly measure on the lowest temperature lattice
...
once on one configuration with high accuracy and a small intial
...
-   Additionally, one should make sure that the stepsizes are small in
...
`binsize * binsize * binsize`. Then the energy-mementum tensor (both the
